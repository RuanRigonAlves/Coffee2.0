<div class="flex flex-col items-center py-2">
    <div class="border-2 rounded border-amber-500 p-2">
        ...
        <form method="POST" action="{{ route('user_info.store') }}" class="flex flex-col items-center text-center">
            ...
            <input type="text" value="{{ old('state') }}" required name="state" id="state"
                class="bg-gray-600">
            @error('state')
                <p>{{ $message }}</p>
            @enderror
            ...
            <input type="text" value="{{ old('city') }}" required name="city" id="city"
                class="bg-gray-600">
            @error('city')
                <p>{{ $message }}</p>
            @enderror
